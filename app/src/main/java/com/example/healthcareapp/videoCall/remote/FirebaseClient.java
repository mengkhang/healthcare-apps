package com.example.healthcareapp.videoCall.remote;
...
public class FirebaseClient {
    ...
    private final DatabaseReference dbRef = FirebaseDatabase.getInstance("https://healthcareapp-deba5-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("videoCall");
    private String currentUseruid;
    private static final String LATEST_EVENT_FIELD_NAME = "latest_event";

    public void login(String useruid, SuccessCallBack callBack){
        dbRef.child(useruid).setValue("").addOnCompleteListener(task -> {
    ...
    public void observeIncomingLatestEvent(NewEventCallBack callBack){
        dbRef.child(currentUseruid).child(LATEST_EVENT_FIELD_NAME).addValueEventListener(
    ...
    public void exit(String useruid) {
        dbRef.child(useruid).removeValue();
    }
}
